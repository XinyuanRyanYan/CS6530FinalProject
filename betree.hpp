...
private:
  ...
  public:
    ...
    mutable uint64_t query_count;
    mutable uint64_t upsert_count;

    // mutable std::queue<int> history;
    // mutable uint64_t history_sum;
    ...
    node()
    {
      ...
      this->epsilon = 0.9; 
      this->creation_time = 1000000 * t.tv_sec + t.tv_usec; // creation timestamp
      this->query_count = 0;
      this->upsert_count = 0;
      // this->history_sum = 0;
    }
    ...
    bool pivots_exceed(betree& bet){
      if (pivots.size() >= pow(bet.max_node_size, this->epsilon))
        return true;
      return false;
    }

    bool elements_exceed(betree& bet){
    ...
    pivot_map flush(betree &bet, message_map &elts)
    {
      ...
      if (is_leaf())
      {
        ...
        // XY: no pivot
        if (elements.size() >= bet.max_node_size)
          result = split(bet);
      ...
      // else
      {
        ...
        while (elements_exceed(bet)) // to-do: epsilon
        {
