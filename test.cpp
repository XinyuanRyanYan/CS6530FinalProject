...
#define DEFAULT_TEST_MAX_NODE_SIZE (1ULL<<8)
#define DEFAULT_TEST_MIN_FLUSH_SIZE (DEFAULT_TEST_MAX_NODE_SIZE / 4)
#define DEFAULT_TEST_CACHE_SIZE (1024)
...
int initialization(betree<uint64_t, std::string> &b,
	 FILE *script_input,
...
{ 
  ...
  std::cout << "upsert_timer: " << upsert_timer << std::endl;
  ...
  if(query_count!=0)
    std::cout << "query avg: " << 1.0*query_timer/query_count << std::endl;
  if(upsert_count!=0)
    std::cout << "upsert avg: " << 1.0*upsert_timer/upsert_count << std::endl;
  std::cout << "Test PASSED: " << std::endl;

  for(unsigned int i = 0; i < 11; i++){
    // std::cout << upsert_timer_segs[i] <<" "<< query_timer_segs[i] <<" "<< overall_timer_segs[i] << std::endl;
  }
  return 0;
}
...
int main(int argc, char **argv)
{
  ...
  // Argument parsing //
  //////////////////////
  // e: epsilon I:min X:max 
  while ((opt = getopt(argc, argv, "m:d:N:f:C:o:k:t:s:i:e:I:A")) != -1) {
    switch (opt) {
    case 'm':
      mode = optarg;
      break;
    case 'e':
      if(optarg[0]=='L')
        epsilon_curve = LINEAR;
      else if(optarg[0]=='S')
        epsilon_curve = SIGMOID;
      else if(optarg[0]=='T')
        epsilon_curve = TANH;
      break;
    case 'I': //  strtof(optarg, NULL)
      min_epsilon = float(strtoull(optarg, &term, 10))/100;
      std::cout << "min_epsilon" << min_epsilon << std::endl;
      break;
    // case 'A':
    //   max_epsilon = float(strtoull(optarg, &term, 10))/100;
  ...
  FILE* init_file = fopen("5winsert.txt", "r");
  if (init_file == NULL) {
      perror("Couldn't open 5winsert file");
      exit(1);
    }
